function BloomFilter(configUrl) {
    ...
    chrome.storage.local.get([
            'blackBloomFilterSourceLocation',
            'blackBloomFilterLastUpdate',
            'blackBloomFilterUpdateInterval',
            'blackBloomFilterVersion',
            'blackBloomFilterCutUriQuery',
            'blackBloomFilterCutUriScheme',
            'blackBloomFilterSize',
            'blackBloomFilterHashes',
            'blackBloomFilterSource'
        ], function(data) {
            if (data.blackBloomFilterVersion && data.blackBloomFilterSource) {
    ...
    function restoreFromLocalStorage(data) {
        sourceLocation = data.blackBloomFilterSourceLocation;
        lastUpdate = data.blackBloomFilterLastUpdate;
        updateInterval = data.blackBloomFilterUpdateInterval;
        version = data.blackBloomFilterVersion;
        cutUriQuery = !!data.blackBloomFilterCutUriQuery;
        cutUriScheme = !!data.blackBloomFilterCutUriScheme;
        size = data.blackBloomFilterSize;
        hashes = data.blackBloomFilterHashes;
        source = data.blackBloomFilterSource;
    }

    function saveToLocalStorage() {
        var objToSave = {
            blackBloomFilterSourceLocation: sourceLocation,
            blackBloomFilterLastUpdate: lastUpdate,
            blackBloomFilterUpdateInterval: updateInterval,
            blackBloomFilterVersion: version,
            blackBloomFilterCutUriQuery: cutUriQuery,
            blackBloomFilterCutUriScheme: cutUriScheme,
            blackBloomFilterSize: size,
            blackBloomFilterHashes: hashes,
            blackBloomFilterSource: source
        };
    ...
    function loadConfigData() {
        ...
        function completeHandler(e) {
            ...
            saveBloomFilterConfig(JSON.parse(e.currentTarget.response).blacklisturldb);
        }
    }
    ...
    function saveBloomFilterConfig(data){
        ...
        if (version < data.version) {
            ...
            lastUpdate = Date.now();
            chrome.storage.local.set({'blackBloomFilterLastUpdate': lastUpdate}, function(result) {});
            ...
            JSZip.loadAsync(evt.currentTarget.response)
                .then(function (a) {
					...
					if(version < chksumVersion) {
						var dbSha1;
						zipFiles["ctrl-06-22-url.db"].async("uint8array").then(function (data) {
	                    	...
	                    	if (dbSha1 == chksumSha1){
		                    	...
		                    	return zipFiles["ctrl-06-22-url.db"].async("string");
		                    }else {
		                    	return null
		                    }
	                    }).then(function (text) {
    ...
    function validateUrl(url) {
    	var index;
        if (cutUriScheme) {
            url = (index = url.indexOf("://")) < 0 ? url : url.substring(index + 3)
        }

        if (cutUriQuery) {
            url = (index = url.indexOf("?")) < 0 ? url : url.substring(0, index)
        }
        return url;
    }
    ...
    public_upi.validateUrl = validateUrl;
